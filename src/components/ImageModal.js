...
export function ImageModal({ image, isOpen, onClose, addOrRemoveImageGallery, isInGallery }) {
    ...
    return (
        <div onClick={onClose} className="fixed inset-0 flex items-center justify-center bg-black/80 z-50">
            <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg p-4 m-4 max-w-[90vw] max-h-[90vh] flex flex-col">
                {/* Header section */}
                ...
                <div className="flex justify-between items-center mt-3">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); addOrRemoveImageGallery(); }} className={`px-4 py-2 ${isInGallery ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600'} text-white rounded transition-colors cursor-pointer`}>
